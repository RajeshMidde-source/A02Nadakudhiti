...
QUnit.test('Testing temparature convertor function with several sets of inputs', function (assert) {
    assert.equal(calc(60, true), 16, 'Tested with one normal positive number, inverse flag true');
    assert.equal(calc(-32, true), -35, 'Tested with one normal negative number, inverse flag true');
    assert.equal(calc(16, false), 61, 'Tested with one normal positive number, inverse flag false');
    assert.equal(calc(35, false), 95, 'Tested with one normal negative number, inverse flag false');
        //throws( block                                    [, expected ] [, message ] ) 
    assert.throws(function () { calc(null,null); }, /input cannot be null/, 'Passing in null correctly raises an Error');
});
